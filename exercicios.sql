...
VALUES ('Informática Biomédica', 'Presencial', 1, 50);
INSERT INTO CURSOS (NOME, CATEGORIA, PROFESSOR, NUMERO_MAX_ALUNOS) 
VALUES ('Medicina', 'EAD', 2, 110);
INSERT INTO CURSOS (NOME, CATEGORIA, PROFESSOR, NUMERO_MAX_ALUNOS) 
VALUES ('Biomedicina', 'Presencial', 3, 30);
...
DELETE FROM MATRICULA_CURSO;
DELETE FROM CURSOS;
ALTER TABLE CURSOS MODIFY PROFESSOR NUMBER
ALTER TABLE CURSOS RENAME COLUMN PROFESSOR TO PROFESSOR_ID;

UPDATE CURSOS 
SET CATEGORIA = 'EAD'
WHERE EXISTS(SELECT 1 FROM PROFESSORES P, ENDERECO E WHERE P.PROFESSOR_ID = CURSOS.PROFESSOR_ID AND P.ENDERECO_ID = E.ENDERECO_ID AND E.ESTADO <> 'SC');
COMMIT;

ALTER TABLE PROFESSORES ADD LOCAL_NASCIMENTO VARCHAR2(100);
ALTER TABLE PROFESSORES ADD SEXO VARCHAR2(1);

UPDATE PROFESSORES
SET LOCAL_NASCIMENTO = 'BRASIL'
COMMIT;

UPDATE PROFESSORES
SET SEXO = 'M'
WHERE PROFESSOR_ID IN (1,3);
UPDATE PROFESSORES
SET SEXO = 'F'
WHERE PROFESSOR_ID IN (2);
COMMIT;

SELECT * FROM CURSOS WHERE NUMERO_MAX_ALUNOS > 30;

SELECT * FROM PROFESSORES WHERE LOCAL_NASCIMENTO IN ('BRASIL', 'ARGENTINA', 'BOLÍVIA', 'CHILE', 'COLÔMBIA', 'EQUADOR', 'GUIANA', 'PARAGUAI', 'PERU', 'SURINAME', 'URUGUAI', 'VENEZUELA');

SELECT * FROM ESTUDANTES WHERE NOME LIKE 'J%';

SELECT * FROM ESTUDANTES WHERE MEDIA_GERAL > 7  ORDER BY MEDIA_GERAL DESC;

SELECT * FROM CURSOS C, PROFESSORES P WHERE C.PROFESSOR_ID = P.PROFESSOR_ID AND P.SEXO = 'F'

SELECT * FROM ESTUDANTES E, CURSOS C, MATRICULA_CURSO M WHERE M.ESTUDANTE_ID = E.ESTUDANTE_ID AND M.CURSO_ID = C.CURSO_ID AND C.CATEGORIA = 'EAD'